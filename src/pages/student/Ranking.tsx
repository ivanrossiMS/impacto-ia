...
export const Ranking: React.FC = () => {
  ...
  const loadRanking = async () => {
    ...
    for (const cls of myClasses) {
      classStudentIds.push(...(cls.studentIds || []));
    }
    
    // Remove duplicates and include current user
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-slate-500 font-medium">Os 10 alunos com maior engajamento e XP da turma.</p>
        </div>
      ...
      {rankData.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-[2.5rem] border-2 border-dashed border-slate-100">
          ...
          <p className="text-slate-400 text-sm">Seja o primeiro a acumular XP na plataforma!</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-8 space-y-8">
              ...
              <div className="grid grid-cols-3 gap-4 items-end pt-12">
                ...
                {top3[1] ? (
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <div className="text-center">
                      <div className="text-sm font-black text-slate-700 truncate max-w-[80px]">{top3[1].user.name.split(' ')[0]}</div>
                ...
                {top3[0] && (
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <div className="text-center">
                      <div className="text-lg font-black text-slate-800 truncate max-w-[90px]">{top3[0].user.name.split(' ')[0]}</div>
                ...
                {top3[2] ? (
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <div className="text-center">
                      <div className="text-sm font-black text-slate-700 truncate max-w-[80px]">{top3[2].user.name.split(' ')[0]}</div>
            ...
            {top10.length > 0 && (
              <Card className="p-4 border-slate-100 rounded-[2.5rem] shadow-xl space-y-2">
                <div className="px-4 py-2 text-[10px] font-black uppercase text-slate-400 tracking-widest border-b border-slate-50 mb-2">
                  Elite da Turma (Exibindo Top 10)
                </div>
                {top10.map((item) => (
                  <div
                    key={item.user.id}
            ...
            {/* If user is not in top 10, show their entry separately */}
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <div>
                        <h4 className="text-base font-black tracking-tight text-primary-700">
                          {myEntry.user.name}
                          <span className="text-[10px] font-black uppercase text-primary-400 ml-2 tracking-widest">(Você)</span>
                        </h4>
                        <div className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest flex items-center gap-2">
              ...
              <div className="relative z-10 space-y-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] font-black text-slate-500 tracking-widest uppercase">Ranking da Turma</div>
                  </div>
                </div>
                ...
                <div className="text-xs font-bold text-slate-500 text-center">
                  Continue completando atividades e missões para subir no ranking!
                </div>
              </div>
            </Card>
          </div>
